...
## Diseño de la base de datos (creación de las tablas, funciones, inserts, etc)
* Crear tabla Rol
Maneja el tipo de rol de la cuenta de usuario
```
CREATE TABLE Rol(
...
```
* Insertar tipos de cuenta en la tabla Rol
```
...
```
* Insertar usuario en tabla Usuarios, importante que la contraseña está encriptado.
```
...
* Crear table Productos: almacena el nombre, foto de productos
```
...
* Crear función para eliminar producto: se valida que el producto existe, se utiliza transacción, de lo contrario se hace un ROLLBACK
```
