...
$this->Cell(5,$textypos,"Inventario actual de productos",0,1,'L');
...
$this->Cell(5,$textypos,"Fecha: ".date("d/m/Y"));
...
// Pie de página
	...
	function Footer() {
		...
		$this->SetFont('Arial', 'B', 10);
		// Número de página
		$this->Cell(170, 10, 'Inventario de productos', 0, 0, 'C', 0);
...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			$this->Cell(30, 8, 'COSTO ACTUAL', 1, 0, 'C', 0);
...
$pdf->Cell(30, 8, 'COSTO ACTUAL', 1, 0, 'C', 0);
...
$pdf->SetWidths(array(30,70,20, 30,30,30)); //???
...
for ($i = 0; $i < $numregs; $i++) {
	...
	$pdf->Row(array(
		mb_convert_encoding(pg_fetch_result($data,$i,'codigoproducto'), "ISO-8859-1", "UTF-8"),
		...
		ucwords(strtolower(mb_convert_encoding(pg_fetch_result($data,$i,'preciocompra'), "ISO-8859-1", "UTF-8" ))) * ucwords(strtolower(mb_convert_encoding(pg_fetch_result($data,$i,'cantidadcomprado'), "ISO-8859-1", "UTF-8")))  
