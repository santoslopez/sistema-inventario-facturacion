...
include '../conexion.php'; //puede que no lo necesiten
...
// Cabecera de página
	...
	function Header() {
		...
		$this->Image('../assets/img/add.png', 10, 10, 20); //imagen(archivo, png/jpg || x,y,tamaño)
		$this->setXY(60, 15);
		$this->Cell(100, 8, 'Factura de compra', 0, 1, 'C', 0);
		$this->Image('../assets/img/add.png', 150, 10, 20); //imagen(archivo, png/jpg || x,y,tamaño)
		$this->Ln(40);
	}
...
// Pie de página
	...
	function Footer() {
		...
		// Número de página
		$this->Cell(170, 10, '1era calle A 4-47', 0, 0, 'C', 0);
...
$pdf->Cell(10, 8, 'Codigo', 1, 0, 'C', 0);
$pdf->Cell(35, 8, 'Cantidad', 1, 0, 'C', 0);
$pdf->Cell(35, 8, 'Costo', 1, 0, 'C', 0);
...
//for ($i = 0; $i < count($data); $i++) {
	...
	//$pdf->Row(array($i + 1, $data[$i]['codInventario'], ucwords(strtolower(utf8_decode($data[$i]['codigoProducto']))), '$' . $data[$i]['cantidadComprado']), 15);
//}
...
while($i<$numregs)
{
    ...
    $pdf->Cell(35,8,$siape1,1,0,'C',$fill);
    $pdf->Cell(35,8,$nome2,1,1,'C',$fill);
    //$fill=!$fill;
    $i++;
}   
